...
import { MediaOwnerType, Prisma, TranslationEntityType } from "@prisma/client";
...
export class BlogService {
  ...
  private sanitizeBlogTranslations(value: unknown) {
  ...
  private async attachMedia<T extends { id: number }>(item: T) {
    const media = await this.prisma.contentMedia.findMany({
      where: {
        ownerType: MediaOwnerType.BLOG,
        ownerId: item.id,
        isActive: true,
      },
  ...
  async findAllPublic() {
    ...
    const visible = posts.filter((item) => this.hasPublicContent(item));
    return Promise.all(visible.map((item) => this.attachMedia(item)));
  }
  ...
  async create(data: CreateBlogPostDto, actor?: AuditActor) {
    ...
    const translations = await this.translations.buildTranslationsForSave({
      ...
      manualTranslations: this.sanitizeBlogTranslations(data.translations),
    });
    ...
    const created = await this.prisma.blogPost.create({
      data: {
        ...
        relatedDestinationSlugs: this.normalizeJsonList(data.relatedDestinationSlugs),
        relatedPropertySlugs: this.normalizeJsonList(data.relatedPropertySlugs),
        relatedExperienceSlugs: this.normalizeJsonList(data.relatedExperienceSlugs),
        relatedPackageSlugs: this.normalizeJsonList(data.relatedPackageSlugs),
        isPublished: Boolean(data.isPublished),
        publishedAt: data.isPublished ? this.parseDate(data.publishedAt) || new Date() : null,
  ...
  async update(id: number, data: UpdateBlogPostDto, actor?: AuditActor) {
    ...
    if (data.publishedAt !== undefined) {
      nextData.publishedAt = this.parseDate(data.publishedAt);
    }
    ...
    if (data.relatedDestinationSlugs !== undefined) {
      nextData.relatedDestinationSlugs = this.normalizeJsonList(
        data.relatedDestinationSlugs
      );
    }
    if (data.relatedPropertySlugs !== undefined) {
      nextData.relatedPropertySlugs = this.normalizeJsonList(data.relatedPropertySlugs);
    }
    if (data.relatedExperienceSlugs !== undefined) {
      nextData.relatedExperienceSlugs = this.normalizeJsonList(
        data.relatedExperienceSlugs
      );
    }
    if (data.relatedPackageSlugs !== undefined) {
      nextData.relatedPackageSlugs = this.normalizeJsonList(data.relatedPackageSlugs);
    }
    ...
    if (data.isPublished !== undefined) {
      ...
      if (nextPublished && !nextData.publishedAt && !previous.publishedAt) {
  ...
  async updateFeature(id: number, isFeatured: boolean, actor?: AuditActor) {
    return this.update(id, { isFeatured }, actor);
  }
