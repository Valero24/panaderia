...
export class PropertiesService {
  ...
  async findAll(features?: string) {
    ...
    const withFeatures = await this.attachPublicFeatures(properties as any[]);

    return withFeatures.map((property: any) => {
      const { destinations, ...propertyData } = property;
  ...
  async create(data: any, actor?: AuditActor) {
    ...
    const translations = await this.translations.buildTranslationsForSave({
      source: {
        ...
        guestRecommendations: data.guestRecommendations || null,
        faq,
      },
    ...
    this.translations.enqueueEntityTranslationInBackground({
      entityType: TranslationEntityType.PROPERTY,
      entityId: created.id,
  ...
  async update(
    ...
    if (data.slug !== undefined || data.title !== undefined) {
      const slug = this.generateSlug(
        String(data.slug || payload.title || current.title)
      );
      slugBase = slug;
    ...
    const changedTranslatableFields = Object.keys(payload).filter((field) =>
    ...
    if (data.translations !== undefined) {
      const translations = await this.translations.buildTranslationsForSave({
        ...
        existingTranslations: (current as any).translations,
        manualTranslations: data.translations,
      });
      ...
      const translationsMeta = this.translations.buildTranslationsMetaForSave({
        manualTranslations: data.translations,
        existingMeta: (current as any).translationsMeta,
      });

      if (translationsMeta !== undefined) {
        payload.translationsMeta = translationsMeta;
      }
    }
    ...
    if (payload.slug !== undefined && slugBase) {
      await recordSlugAudit({
        audit: this.audit,
        actor,
        entityType: "Property",
